...
**邮箱撞上已有的本地账号。** 返回 `403`，且什么都不写。

同一个邮箱在两个身份域都「已验证」，只证明两边都认为它可达，**不证明两个域里的主体
是同一个人**：邮箱会被回收、会随换工作转手、企业域名会换归属。按这个依据建立关联，
等于让任何能注册到同名地址的人够到一个既有账号。

要给已有账号加一个 provider，先登录那个账号再显式绑定 —— 主体先认证、provider 再
认证，两边都成立之后才建立绑定。<Status kind="tested" guard="conformance::a4" />
